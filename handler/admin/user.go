...
func (u *UserHandler) GetCurrentUserProfile(ctx *fiber.Ctx) (interface{}, error) {
	...
	return u.UserService.ConvertToDTO(ctx.Context(), user), nil
}
...
func (u *UserHandler) UpdateUserProfile(ctx *fiber.Ctx) (interface{}, error) {
	...
	user, err := u.UserService.Update(ctx.Context(), userParam)
	if err != nil {
		return nil, err
	}
	return u.UserService.ConvertToDTO(ctx.Context(), user), nil
...
func (u *UserHandler) UpdatePassword(ctx *fiber.Ctx) (interface{}, error) {
	...
	return nil, u.UserService.UpdatePassword(ctx.Context(), passwordParam.OldPassword, passwordParam.NewPassword)
}
...
func (u *UserHandler) GenerateMFAQRCode(ctx *fiber.Ctx) (interface{}, error) {
	...
	if *param.MFAType == consts.MFATFATotp {
		key, url, err := u.TwoFactorMFAService.GenerateOTPKey(ctx.Context(), user.Nickname)
		if err != nil {
			return nil, err
		}
		...
		qrCode, err := u.TwoFactorMFAService.GenerateMFAQRCode(ctx.Context(), url)
		if err != nil {
			return nil, err
		}
...
func (u *UserHandler) UpdateMFA(ctx *fiber.Ctx) (interface{}, error) {
	...
	return nil, u.UserService.UpdateMFA(ctx.Context(), mfaParam.MFAKey, *mfaParam.MFAType, mfaParam.AuthCode)
}
