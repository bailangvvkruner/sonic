...
func (v *ViewHandler) Install(ctx *fiber.Ctx) error {
	isInstall := v.OptionService.GetOrByDefault(ctx.Context(), property.IsInstalled).(bool)
	if isInstall {
		return nil
	}
	adminURLPath, _ := v.OptionService.GetAdminURLPath(ctx.Context())
...
func (v *ViewHandler) Logo(ctx *fiber.Ctx) (interface{}, error) {
	logo := v.OptionService.GetOrByDefault(ctx.Context(), property.BlogLogo).(string)
	if logo != "" {
...
func (v *ViewHandler) Favicon(ctx *fiber.Ctx) (interface{}, error) {
	favicon := v.OptionService.GetOrByDefault(ctx.Context(), property.BlogFavicon).(string)
	if favicon != "" {
...
func (v *ViewHandler) Authenticate(ctx *fiber.Ctx, model template.Model) (string, error) {
	...
	err = ctx.BodyParser(&authenticationParam)
	if err != nil {
...
func (v *ViewHandler) authenticateCategory(ctx *fiber.Ctx, slug, password, token string) (string, error) {
	category, err := v.CategoryService.GetBySlug(ctx.Context(), slug)
	if err != nil {
		return "", err
	}
	categoryDTO, err := v.CategoryService.ConvertToCategoryDTO(ctx.Context(), category)
	if err != nil {
		return "", err
	}

	token, err = v.CategoryAuthentication.Authenticate(ctx.Context(), token, category.ID, password)
	if err != nil {
		return "", err
	}

	return token, ctx.Redirect(categoryDTO.FullPath, http.StatusFound)
}

func (v *ViewHandler) authenticatePost(ctx *fiber.Ctx, slug, password, token string) (string, error) {
	post, err := v.PostService.GetBySlug(ctx.Context(), slug)
	if err != nil {
		return "", err
	}
	fullPath, err := v.PostService.BuildFullPath(ctx.Context(), post)
	if err != nil {
		return "", err
	}
	token, err = v.PostAuthentication.Authenticate(ctx.Context(), token, post.ID, password)
	if err != nil {
		return "", err
	}

	return token, ctx.Redirect(fullPath, http.StatusFound)
}
...
func (v *ViewHandler) authenticateErr(ctx *fiber.Ctx, model template.Model, aType string, slug string, err error) (string, error) {
	...
	if exist, err := v.ThemeService.TemplateExist(ctx.Context(), "post_password.tmpl"); err == nil && exist {
		return v.ThemeService.Render(ctx.Context(), "post_password")
	}
