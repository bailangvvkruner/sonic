...
func (a *ArchiveHandler) Archives(ctx *fiber.Ctx, model template.Model) (string, error) {
	return a.PostModel.Archives(ctx.Context(), 0, model)
}
...
func (a *ArchiveHandler) ArchivesPage(ctx *fiber.Ctx, model template.Model) (string, error) {
	...
	return a.PostModel.Archives(ctx.Context(), int(page-1), model)
}
...
func (a *ArchiveHandler) ArchivesBySlug(ctx *fiber.Ctx, model template.Model) (string, error) {
	...
	postPermalinkType, err := a.OptionService.GetPostPermalinkType(ctx.Context())
	if err != nil {
		return "", err
	}
	var post *entity.Post
	if postPermalinkType == consts.PostPermalinkTypeDefault {
		post, err = a.PostService.GetBySlug(ctx.Context(), slug)
		if err != nil {
			return "", err
		}
		...
		post, err = a.PostService.GetByPostID(ctx.Context(), int32(postID))
		if err != nil {
			return "", err
		}
	}
	token := ctx.Cookies("authentication")
	return a.PostModel.Content(ctx.Context(), post, token, model)
}
...
func (a *ArchiveHandler) AdminArchivesBySlug(ctx *fiber.Ctx, model template.Model) (string, error) {
	...
	return a.PostModel.AdminPreview(ctx.Context(), post, model)
}
