...
func (f *FeedHandler) Atom(ctx *fiber.Ctx, model template.Model) (string, error) {
	rssPageSize := f.OptionService.GetOrByDefault(ctx.Context(), property.RssPageSize).(int)
	...
	posts, _, err := f.PostService.Page(ctx.Context(), postQuery)
	if err != nil {
		return "", err
	}
	postDetailVOs, err := f.buildPost(ctx.Context(), posts)
	if err != nil {
		return "", err
	}
...
func (f *FeedHandler) CategoryAtom(ctx *fiber.Ctx, model template.Model) (string, error) {
	...
	category, err := f.CategoryService.GetBySlug(ctx.Context(), slug)
	if err != nil {
		return "", err
	}
	categoryDTO, err := f.CategoryService.ConvertToCategoryDTO(ctx.Context(), category)
	if err != nil {
		return "", err
	}

	posts, err := f.PostCategoryService.ListByCategoryID(ctx.Context(), category.ID, consts.PostStatusPublished)
	if err != nil {
		return "", err
	}
...
func (f *FeedHandler) SitemapXML(ctx *fiber.Ctx, model template.Model) (string, error) {
	posts, _, err := f.PostService.Page(ctx.Context(), param.PostQuery{
