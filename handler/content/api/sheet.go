...
func (s *SheetHandler) ListTopComment(ctx *fiber.Ctx) (interface{}, error) {
	...
	pageSize := s.OptionService.GetOrByDefault(ctx.Context(), property.CommentPageSize).(int)
	...
	comments, totalCount, err := s.SheetCommentService.Page(ctx.Context(), commentQuery, consts.CommentTypeSheet)
	if err != nil {
		return nil, err
	}
	_ = s.SheetCommentAssembler.ClearSensitiveField(ctx.Context(), comments)
	commenVOs, err := s.SheetCommentAssembler.ConvertToWithHasChildren(ctx.Context(), comments)
	if err != nil {
		return nil, err
	}
...
func (s *SheetHandler) ListChildren(ctx *fiber.Ctx) (interface{}, error) {
	...
	children, err := s.SheetCommentService.GetChildren(ctx.Context(), parentID, sheetID, consts.CommentTypeSheet)
	if err != nil {
		return nil, err
	}
	_ = s.SheetCommentAssembler.ClearSensitiveField(ctx.Context(), children)
	return s.SheetCommentAssembler.ConvertToDTOList(ctx.Context(), children)
}
...
func (s *SheetHandler) ListCommentTree(ctx *fiber.Ctx) (interface{}, error) {
	...
	allComments, err := s.SheetCommentService.GetByContentID(ctx.Context(), sheetID, consts.CommentTypeSheet, commentQuery.Sort)
	if err != nil {
		return nil, err
	}
	_ = s.SheetCommentAssembler.ClearSensitiveField(ctx.Context(), allComments)
	commentVOs, total, err := s.SheetCommentAssembler.PageConvertToVOs(ctx.Context(), allComments, commentQuery.Page)
	if err != nil {
		return nil, err
	}
...
func (s *SheetHandler) ListComment(ctx *fiber.Ctx) (interface{}, error) {
	...
	comments, total, err := s.SheetCommentService.Page(ctx.Context(), commentQuery, consts.CommentTypeSheet)
	if err != nil {
		return nil, err
	}
	_ = s.SheetCommentAssembler.ClearSensitiveField(ctx.Context(), comments)
	result, err := s.SheetCommentAssembler.ConvertToWithParentVO(ctx.Context(), comments)
	if err != nil {
		return nil, err
	}
...
func (s *SheetHandler) CreateComment(ctx *fiber.Ctx) (interface{}, error) {
	...
	result, err := s.SheetCommentService.CreateBy(ctx.Context(), &comment)
	if err != nil {
		return nil, err
	}
	return s.SheetCommentAssembler.ConvertToDTO(ctx.Context(), result)
}
