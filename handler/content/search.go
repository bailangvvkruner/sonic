...
func (s *SearchHandler) search(ctx *fiber.Ctx, pageNum int, model template.Model) (string, error) {
	...
	if len(sort.Fields) == 0 {
		sort = s.OptionService.GetPostSort(ctx.Context())
	}
	defaultPageSize := s.OptionService.GetIndexPageSize(ctx.Context())
	page := param.Page{
		PageNum:  pageNum,
	...
	posts, total, err := s.PostService.Page(ctx.Context(), postQuery)
	if err != nil {
		return "", err
	}
	postVOs, err := s.PostAssembler.ConvertToListVO(ctx.Context(), posts)
	if err != nil {
		return "", err
	}
	...
	model["meta_keywords"] = s.OptionService.GetOrByDefault(ctx.Context(), property.SeoKeywords)
	model["meta_description"] = s.OptionService.GetOrByDefault(ctx.Context(), property.SeoDescription)
	return s.ThemeService.Render(ctx.Context(), "search")
}
